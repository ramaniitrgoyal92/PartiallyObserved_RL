		...
		if path is None:
			...
			for t in range(0, self.N):
				...
				if init is None:
					...
					self.U_p[t, :] = init[t,:].reshape((self.n_u,1))
				
			np.copyto(self.U_p_temp, self.U_p)
			# print('unom_temp check = ', self.U_p_temp)
			# print('init = ', init)
...
if __name__=="__main__":
	...
	MODEL_XML = path_to_model_free_DDP + "/models/fish_old.xml"
	path_to_exp = path_to_model_free_DDP + "/experiments/Material/exp_burgers"
	...
	if training_flag_on:
		...
		print(model.X_p[-1])
		np.save('burgers_traj.npy',model.X_p)
		...
		plt.figure()
		plt.plot(np.linspace(-1, 1, state_dimension), initial_state, 'b')
		plt.plot(np.linspace(-1, 1, state_dimension), model.X_p[int(horizon/2),:], '--r')
		plt.plot(np.linspace(-1, 1, state_dimension), model.X_p[-1], 'g')
