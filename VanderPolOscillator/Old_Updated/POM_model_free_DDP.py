...
from material_params import C, q, q_u, obs_dimension, n_ac, n_ch, dt_ch
...
class DDP(object):
	...
	def __init__(self, MODEL_XML, n_x, n_u, alpha, horizon, initial_state, final_state):
		...
		self.K = np.zeros((self.N, self.n_u, obs_dimension*q+self.n_u*(q_u-1)))
		self.k = np.zeros((self.N, self.n_u, 1))
		
		self.V_zz = np.zeros((self.N, obs_dimension*q+self.n_u*(q_u-1), obs_dimension*q+self.n_u*(q_u-1)))#np.zeros((self.N, self.n_x, self.n_x))
		self.V_z = np.zeros((self.N, obs_dimension*q+self.n_u*(q_u-1), 1))#np.zeros((self.N, self.n_x, 1))
	...
	def partials_list_arma(self, x_0, u_nom, V_z, V_zz, activate_second_order_dynamics, finite_difference_gradients_flag=False, del_J_alpha=0, b_pass_success_flag=1):	
		...
		if finite_difference_gradients_flag:
			...
			A_aug, B_aug, V_z_F_XU_XU, traj = self.sys_id(x_0, u_nom, central_diff=1, activate_second_order=activate_second_order_dynamics, V_z=V_z, p_mask=self.p_mask, m_mask=self.m_mask)
	...
	def forward_pass_sim(self, render=0, std=0, pr=False):
		...
		# dx = (2*np.pi)/self.n_x
		# dt = 1e-4
		# nu = 0.1#0.01/np.pi#0.01/np.pi
		# tperiod = 1000
		d_z = np.zeros((obs_dimension*q,1))
		d_u = np.zeros((self.n_u*q_u,1))
		#print((self.X_p_0))
		#K*[dz_seq(n_z*(horizon-i+1)+1:n_z*(horizon-i+1+q),1);du_seq(n_u*(horizon-i+2)+1:n_u*(horizon-i+1+q_u),1)]
		...
		for t in range(0, self.N):
			...
			if t<max(q, q_u):
				...
				self.U_p[t] = self.U_p_temp[t] + self.alpha*self.k[t]# + np.random.normal(np.zeros(np.shape(self.U_ep_temp[t])), std)
				d_u[self.n_u*(q_u-t-1):self.n_u*(q_u-t)] = self.U_p[t]-self.U_p_temp[t]
				if t is not 0:
					d_z[obs_dimension*(q-t-1):obs_dimension*(q-t)] = C @ (self.X_p[t-1] - self.X_p_temp[t-1])	
				
			else:
				d_z_prev = d_z[:obs_dimension*(q-1)]
				d_z[obs_dimension:] = d_z_prev
				d_u_prev = d_u[:self.n_u*(q_u-1)]
				d_u[self.n_u:] = d_u_prev

				d_z[:obs_dimension] = C @ (self.X_p[t-1] - self.X_p_temp[t-1])
				
				# print('X at '+str(t)+' = ', self.X_p[t-1])
				# print('X_temp at '+str(t)+' = ', self.X_p_temp[t-1])
				self.U_p[t] = self.U_p_temp[t] + self.alpha*self.k[t] + (self.K[t] @ np.vstack([d_z, d_u[self.n_u:]]))
				d_u[:self.n_u] = self.U_p[t]-self.U_p_temp[t]
			 
			# print('Control at '+str(t)+' = ', self.U_p[t])
			...
			if t is 0:	
				...
				# self.X_p[t] = PFM_CH.simulation(n_ch, 0.001, dt_ch, ctrl[:int(len(ctrl)/2)].reshape((xdim, xdim)), ctrl[int(len(ctrl)/2):].reshape((xdim, xdim)), self.X_p_0.reshape((xdim, xdim))).reshape((xdim*xdim, 1))
				...
				# self.X_p[t] = PFM_CH.simulation(n_ch, 0.001, dt_ch, ctrl[:int(len(ctrl)/2)].reshape((xdim, xdim)), ctrl[int(len(ctrl)/2):].reshape((xdim, xdim)), self.X_p[t-1].reshape((xdim, xdim))).reshape((xdim*xdim, 1))
