...
class SimulateCartPole:
    ...
    def _angleNormalize(self, theta):
        ...
        return theta#((theta + np.pi) % (2 * np.pi)) - np.pi
    ...
    def onestep_rk4(self, state_init, n_per_step, u=0.0):
        """
        ...
        for i in range(n_per_step):
            # RK4 integration
            k1 = self.cartpole_dynamics(state, u)
            k2 = self.cartpole_dynamics(state + 0.5 * h * k1, u)
            k3 = self.cartpole_dynamics(state + 0.5 * h * k2, u)
            k4 = self.cartpole_dynamics(state + h * k3, u)
    ...
    def simulate(self, x_init, u_scalar):
        ...
        # Single RK4 step
        k1 = self.cartpole_dynamics(x_init, u_scalar)
        k2 = self.cartpole_dynamics(x_init + 0.5 * self.dt * k1, u_scalar)
        k3 = self.cartpole_dynamics(x_init + 0.5 * self.dt * k2, u_scalar)
        k4 = self.cartpole_dynamics(x_init + self.dt * k3, u_scalar)
    ...
    def draw_figure(self, save_to_path=None):
        ...
        plt.suptitle('CartPole Swing-Up Trajectory', fontsize=14, fontweight='bold')
        plt.tight_layout()
...
if __name__ == '__main__':
    ...
    save_path = file_loc / "cartpole_trajectory.png"
    ...
    print('='*70)
    print('CARTPOLE SWING-UP SIMULATION')
    print('='*70)
